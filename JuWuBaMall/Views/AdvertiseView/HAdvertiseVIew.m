...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        [_scrollView setContentSize:CGSizeMake(_adList.count * self.width, self.height)];
        ...
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(self.width-60 , self.bounds.size.height-20, 0, 20)];
        ...
        _pageControl.pageIndicatorTintColor = kMainBackGroundColor;
...
//        [_schemaUrlArr addObject:advertiseInfo.schemaUrl];
      ...
      // [_imagesUrlArr addObject:LoadImageUrl(adInfo.imageUrl)];
        [_imagesUrlArr addObject:adInfo.imageUrl];

        NSString *imageName = [NSString stringWithFormat:@"test%d",i];
        
        UIImageView *imageView = [[UIImageView alloc] init];
        imageView.image = [UIImage imageNamed:imageName];
        [imageView sd_setImageWithURL:[NSURL URLWithString:adInfo.imageUrl]
...
- (void)handleSwipeGesture:(UISwipeGestureRecognizer *)swipe
{
    ...
    LogInfo(@"草，滑动啦！！！！");
     UISwipeGestureRecognizerDirection direction=[(UISwipeGestureRecognizer*) swipe direction];
    ...
    switch (direction) {
        ...
        case UISwipeGestureRecognizerDirectionRight:
            LogInfo(@"向右滑动啦");
            CGPoint nOffset = CGPointMake(_scrollView.contentOffset.x - CGRectGetWidth(_scrollView.frame), _scrollView.contentOffset.y);
            [_scrollView setContentOffset:nOffset animated:YES];
